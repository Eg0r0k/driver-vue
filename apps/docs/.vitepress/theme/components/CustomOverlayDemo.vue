...
const { drive } = useDriver({
  stagePadding: 8,
  steps: [
    { element: "#ovl-title", popover: { title: "Frosted overlay", description: "Four blurred panels instead of an SVG path." } },
    { element: "#ovl-export", popover: { title: "Still animated", description: "The panels are driven by the same stage rect.", side: "right" } },
  ],
});

function panels(stage: { x: number; y: number; width: number; height: number }, padding: number) {
