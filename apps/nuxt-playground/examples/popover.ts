...
export const popoverGroup: ExampleGroup = {
  ...
  intro: "Which buttons a popover shows, what they say, what they do, and how the popover is styled.",
  examples: [
    {
      id: "no-buttons",
      title: "No Buttons",
      description: "A bare highlight shows no buttons by default.",
      run: ctx => {
    ...
    },
    {
      id: "next-button",
      title: "Next Button",
      description: "Show only the next and close buttons; the next click is handled by a hook.",
      run: ctx => {
        ctx.configure().highlight({
          element: "#card-3",
          popover: {
            ...
            onNextClick: (element, step, opts) => {
              ctx.log("Next Clicked", element, step);
              opts.driver.destroy();
            },
          },
        });
    ...
    },
    {
      id: "button-texts",
      title: "Button Texts",
      description: "Override the button labels from the config and from the popover.",
      run: ctx => {
        ctx.configure({ prevBtnText: "<——", nextBtnText: "——>" }).highlight({
          element: "#card-1",
          popover: {
            side: "left",
    ...
    },
    {
      ...
      description: "Listen to button clicks globally; the popover can override a listener per step.",
      run: ctx => {
        const driverObj = ctx.configure({
          onNextClick: () => ctx.notice("Next Clicked"),
          onPrevClick: () => ctx.notice("Previous Clicked"),
        ...
        driverObj.highlight({
          popover: {
            ...
            showButtons: ["close", "next", "previous"],
            onPrevClick: () => ctx.notice("Overriding: Previous Clicked"),
          },
        });
      },
    },
    {
      ...
      description: "Apply a custom class to style the popover and its buttons.",
      run: ctx => {
        ctx.configure({ popoverClass: "custom-driver-popover" }).highlight({
          popover: {
    ...
    },
    {
      id: "popover-hook",
      title: "Popover Hook",
      description: "Modify the popover DOM in the onPopoverRender hook; the step-level hook wins over the global one.",
      run: ctx => {
        ctx
          .configure({
    ...
    },
    {
      ...
      description: "Append a custom button to the popover footer on every step.",
      run: ctx => {
        ctx
          .configure({
            onPopoverRender: popover => {
              ...
              firstButton.addEventListener("click", () => ctx.log("First Button Clicked"));
            },
            steps: [
    ...
    },
    {
      ...
      run: ctx => {
        ctx.configure({ stagePadding: 0, popoverOffset: 20, stageRadius: 10 }).highlight({
          element: "#card-3",
          popover: {
            title: "Padding & Radius",
